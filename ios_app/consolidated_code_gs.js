...
function syncGoogleFitData() {
  ...
  var now = new Date();
  var torontoStr = Utilities.formatDate(now, "America/New_York", "yyyy-MM-dd'T'00:00:00XXX");
  ...
  sleepSessions.forEach(function(session) {
    ...
    var wakeTimeStr = Utilities.formatDate(wakeDate, "America/New_York", "hh:mm a");
          ...
          if (itemDate) {
            ...
            if (timeRaw) {
              if (timeRaw instanceof Date) {
                timeFormatted = Utilities.formatDate(timeRaw, "America/New_York", "HH:mm:ss");
              } else {
...
function doPost(e) {
  try {
    ...
    if (action === "upload_biometrics") {
      ...
      var tzVal = getParam(params, ["timezone", "tz"]);
      var tz = tzVal !== undefined && tzVal !== null ? String(tzVal).trim() : "America/New_York";
      
      var wakeTimeVal = getParam(params, ["wakeTime", "wake_time", "wake", "wakeUpTime", "wake_up_time"]);
      var wakeTime = wakeTimeVal !== undefined && wakeTimeVal !== null ? formatTimeValue(String(wakeTimeVal).trim(), tz) : null;
      if (wakeTime === "") wakeTime = null;

      var sleepTimeVal = getParam(params, ["sleepTime", "sleep_time", "sleep_start", "sleepstart"]);
      var sleepTime = sleepTimeVal !== undefined && sleepTimeVal !== null ? formatTimeValue(String(sleepTimeVal).trim(), tz) : null;
    ...
    if (action === "log_workout") {
      ...
      var timestamp = Utilities.formatDate(now, "America/New_York", "HH:mm:ss");
    ...
    if (action === "import_hevy_csv") {
      ...
      for (var i = 1; i < lines.length; i++) {
        ...
        if (!isNaN(parsedDate.getTime())) {
          timestamp = Utilities.formatDate(parsedDate, "America/New_York", "HH:mm:ss");
        }
    ...
    if (action === "log_location") {
      ...
      var timestamp = params.timestamp || Utilities.formatDate(now, "America/New_York", "yyyy-MM-dd HH:mm:ss");
